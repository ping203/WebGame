...
public class SamChooseCard {
    ...
    private static int[] nhacsanhAll (int indexFocus, int[] cardHand) {
        ...
        for(int i = indexFocus; i < cardHand.Length; i++) {
            if((getValue (cardHand[i])) == valueCurrent + 1) {
                ...
                i = (i + 1) >= size ? -1 : i;
                continue;
            } else if((getValue (cardHand[i])) == valueCurrent) {
                i = (i + 1) >= size ? -1 : i;
                continue;
            }
            break;
        ...
        try {
            ...
            switch(typeC) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    // indexCardNhac = nhac4doithong(indexFocus, cardHand,
                    // cardFire);
                    // 4 doi thong
                    break;
                case 10:
                    if(getValue (cardHand[indexFocus]) == 2) {
                        ...
                        // cardFire);
                        // if (indexCardNhac == null) {
                        // indexCardNhac = nhactuquy(indexFocus, cardHand,
                        // cardFire);
                        // } else {
                        // if (indexCardNhac.Length == 0) {
                        // indexCardNhac = nhactuquy(indexFocus, cardHand,
    ...
    public static bool issanh (int[] card) {
        for(int i = 0; i < card.Length - 1; i++) {
            ...
            if(getValue (card[i]) + 1 == getValue (card[i + 1])) {
                continue;
            } else if(getValue (card[i + 1]) == getValue (card[0]) - 1) {
                continue;
            } else if(i < card.Length - 2 && (getValue (card[i + 1] + 1) == getValue (card[i + 2]))) {
                continue;
            } else {
                return false;
            }
    ...
    private static int[] nhacsanh (int indexFocus, int[] cardHand, int[] cardFire) {
        ...
        if(sanh != null && sanh.Length == cardFire.Length && compareCard (cardUp, cardFire)
                && getValue (cardFire[0]) != getValue (sanh[sanh.Length - 1])) {
